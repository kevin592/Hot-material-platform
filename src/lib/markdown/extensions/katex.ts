...
function createRenderer(display: boolean, inlineStyle: string, blockStyle: string, withStyle: boolean = true) {
  return (token: any) => {
    ...
    if (typeof window === 'undefined' || !window.MathJax || !window.MathJax.tex2svg) {
      // MathJax未加载,返回原始LaTeX代码
      const fallbackText = display ? `$$${token.text}$$` : `$${token.text}$`
      return `<code style="color: #666; background: #f5f5f5; padding: 2px 4px; border-radius: 3px;">${fallbackText}</code>`
    }
